...
private:
	...
	void mainLoop(GLFWwindow* window, Scene& scene) {
		while (!glfwWindowShouldClose(window) && !glfwGetKey(window, GLFW_KEY_ESCAPE)) {
			...
			//If id value == 1 true
			deferID = glfwGetKey(window, GLFW_KEY_1);
			if (deferID == 1) {
				cout << "Current shader: Deferred shading" << endl;
				scene.shaderSelection = 1;
			}

			HdrID = glfwGetKey(window, GLFW_KEY_2);
			if (HdrID == 1) {
				cout << "Current shader: HDR rendering" << endl;
				scene.shaderSelection = 2;
			}

			waveID = glfwGetKey(window, GLFW_KEY_3);
			if (waveID == 1) {
				cout << "Current shader: Wave shader running" << endl;
				scene.shaderSelection = 3;
			}
